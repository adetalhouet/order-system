...
class CartSchemaModule : SchemaModule() {
    ...
    @Query("addToCart")
    suspend fun addToCart(client: CartServiceGrpcKt.CartServiceCoroutineStub, request: CartItem): Status {
        return client.addToCart(request)
    }

    @Query("remoteFromCart")
    suspend fun remoteFromCart(client: CartServiceGrpcKt.CartServiceCoroutineStub, request: CartItem): Status {
